  ...
  const void * argVal )
{
  ...
  this->m_KernelArgumentReady[ kernelId ][ argId ].m_GPUDataManager = NULL;
...
OpenCLKernelManager::CheckArgumentReady( const std::size_t kernelId )
{
  ...
  for( std::size_t i = 0; i < nArg; i++ )
  {
    ...
    if( this->m_KernelArgumentReady[ kernelId ][ i ].m_GPUDataManager.IsNull() )
    {
...
OpenCLKernelManager::ResetArguments( const std::size_t kernelIdx )
{
  ...
  for( std::size_t i = 0; i < nArg; i++ )
  {
    ...
    this->m_KernelArgumentReady[ kernelIdx ][ i ].m_GPUDataManager = NULL;
  }
}
