...
::GPUBSplineDecompositionImageFilter()
{
  ...
  if( !program.IsNull() )
  {
    this->m_FilterGPUKernelHandle
      = this->m_GPUKernelManager->CreateKernel( program, "BSplineDecompositionImageFilter" );
  }
  else
  {
...
::GPUGenerateData( void )
{
  ...
  const typename GPUInputImage::Pointer inPtr
    ...
    switch( ImageDim )
    {
      case 1:
      {
        OpenCLEvent event = this->m_GPUKernelManager->LaunchKernel(
          this->m_FilterGPUKernelHandle, OpenCLSize( 1 ), OpenCLSize( 1 ) );
