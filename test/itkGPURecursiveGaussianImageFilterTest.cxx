...
main( int argc, char * argv[] )
{
  ...
  itk::CreateOpenCLLogger( "RecursiveGaussianImageFilterTest" );
  ...
  parser->GetCommandLineArgument( "-direction", parameters.direction );
...
ProcessImage( const Parameters & _parameters )
{
  ...
  if( !_parameters.skipCPU )
  {
    ...
    if( !updateExceptionCPU )
    {
      for( unsigned int i = 0; i < _parameters.runTimes; i++ )
      {
        ...
        CPUFilter->SetDirection( _parameters.direction );
        CPUFilter->SetSigma( _parameters.sigma );
        CPUFilter->Update();
  ...
  if( !_parameters.skipGPU )
  {
    ...
    catch( itk::OpenCLCompileError & e )
    {
      ...
      std::cerr << "ERROR: OpenCL program has not been compiled"
                << " during GPU filter creation." << std::endl
    ...
    if( !updateExceptionGPU )
    {
      for( unsigned int i = 0; i < _parameters.runTimes; i++ )
      {
        ...
        GPUFilter->SetDirection( _parameters.direction );
        GPUFilter->SetSigma( _parameters.sigma );
