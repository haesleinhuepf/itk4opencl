...
GetLogFileName()
{
  ...
  std::replace( deviceName.begin(), deviceName.end(), ' ', '-' );   // replace
                                                                    // spaces

  deviceName.erase( deviceName.end() - 1, deviceName.end() );     // remove end
                                                                  // of line
...
class ImageToImageFilterType, typename OutputImage >
void
GetTestFilterResult(
  ...
  TScalarType & rmsRelative )
{
    ...
    !cit.IsAtEnd(); ++cit, ++git, ++mcit, ++mgit )
  {
    if( ( mcit.Get() == NumericTraits< typename MaskImageType::PixelType >::One )
      && ( mgit.Get() == NumericTraits< typename MaskImageType::PixelType >::One ) )
    {
      TScalarType cpu = static_cast< TScalarType >( cit.Get() );
      TScalarType err = cpu - static_cast< TScalarType >( git.Get() );
      rmse          += err * err;
  ...
  const std::string & comments = "" )
{
  ...
  if( !fileExists )
  {
    fout << "Filter Name" << s << "Dimension" << s << "Image Size"
         ...
         << s << "RMS Error" << s << "RMS Relative" << s << "Test Passed" << s << "Run Times" << s << "Comments"
         << std::endl;
  }
